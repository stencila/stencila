 ...
 */
export function registerSetupView(
  context: vscode.ExtensionContext,
  client: LanguageClient
) {
  const treeDataProvider = new SetupTreeProvider(context, client);
...
class SetupTreeItem extends vscode.TreeItem {
  constructor(what: 'stencila-cloud' | 'ai-api-key' | 'user-details', checked: boolean) {
    const [label, tip, value] = (() => {
      switch(what) {
        case 'stencila-cloud':
          return checked ?
            ['Sign in to Stencila Cloud', 'Signed in to Stencila Cloud and using our smart model router and other services.', 'stencila-cloud-signed-in'] :
            ['Sign in to Stencila Cloud', 'Sign in to Stencila Cloud to use our smart model router and other services.', 'stencila-cloud-signed-out']
        case 'ai-api-key':
          return checked ? 
            ['Add an AI provider API key', 'One or more API keys added for use with AI commands.', 'ai-api-key-some'] :
            ['Add an AI provider API key', 'Add an API key from an AI model provider. Only necessary if not signed in to Stencila Cloud.', 'ai-api-key-none']
        case 'user-details':
          return checked ? 
            ['Set user details', 'User details have been set and are used to attribute authorship.', 'user-details'] :
            ['Set user details', 'Set user details (e.g. name, affiliation) to use when attributing authorship.', 'user-details']
      }
    })();
    super(label)

    this.tooltip = tip
    this.iconPath = new vscode.ThemeIcon(checked ? 'pass-filled' : 'circle-large-outline')
...
class SetupTreeProvider implements vscode.TreeDataProvider<SetupTreeItem> {
  ...
  async refresh(): Promise<void> {
    ...
    this.items = stencilaCloud || aiApiKey || userDetails ? [
  ...
  async getChildren(item?: SetupTreeItem): Promise<SetupTreeItem[]> {
    return this.items;
  }
}
