...
pub async fn call<Params, Output>(
    method: &str,
    params: Params,
    ...
    Output: DeserializeOwned,
{
    ...
    if let Some(error) = response.remove("error").filter(|error| !error.is_null()) {
        let error = serde_json::from_value::<JsonRpcError>(error).map_err(Error::transport)?;
        return Err(Error {
            code: error.code,
    ...
    serde_json::from_value(result).map_err(Error::transport)
}
