...
This crate (`stencila-models3`) is a **greenfield** Rust implementation of the [Unified LLM Client Specification](https://github.com/strongdm/attractor/blob/main/unified-llm-spec.md). A copy of the spec is in `specs/unified-llm-spec.md`.

It will supersede the existing `stencila-models` crate for Stencila v3. **Do NOT reference or get distracted by the existing sibling `stencila-*` crates** — treat this as an independent implementation of the spec.
...
- **Always consult `specs/unified-llm-spec.md`** before implementing or modifying a feature. The spec is the source of truth.
