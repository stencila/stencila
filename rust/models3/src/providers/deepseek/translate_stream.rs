...
/// Stateful translator for DeepSeek Chat Completions SSE events.
#[derive(Debug, Clone)]
pub struct DeepSeekStreamState(ChatCompletionsStreamState);

impl Default for DeepSeekStreamState {
    fn default() -> Self {
...
pub fn translate_sse_stream<'a>(
    ...
    crate::providers::common::chat_completions::translate_sse_stream(
        sse_stream,
        ChatCompletionsStreamState::new("deepseek", rate_limit, ERROR_CONFIG),
    )
}
