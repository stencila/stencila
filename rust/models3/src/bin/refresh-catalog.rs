...
//!
//! Fetches model listings from provider APIs and merges newly discovered
//! models into `src/catalog/models.json`.
//!
//! Usage:
...
async fn main() {
    ...
    let catalog_path = manifest_dir.join("src/catalog/models.json");
    ...
    if let Ok(api_key) = std::env::var("ANTHROPIC_API_KEY") {
        match fetch_anthropic_models(&client, &api_key).await {
            Ok(models) => {
                added += merge_models(&mut catalog, &mut known_keys, models);
            }
            Err(e) => eprintln!("Anthropic model list failed: {e}"),
        }
    }
    ...
    {
        match fetch_gemini_models(&client, &api_key).await {
            Ok(models) => {
                added += merge_models(&mut catalog, &mut known_keys, models);
            }
            Err(e) => eprintln!("Gemini model list failed: {e}"),
        }
    }

    // OpenAI
    if let Ok(api_key) = std::env::var("OPENAI_API_KEY") {
        match fetch_openai_models(&client, &api_key).await {
            Ok(models) => {
                added += merge_models(&mut catalog, &mut known_keys, models);
            }
            Err(e) => eprintln!("OpenAI model list failed: {e}"),
        }
    }

    // Mistral
    if let Ok(api_key) = std::env::var("MISTRAL_API_KEY") {
        match fetch_mistral_models(&client, &api_key).await {
            Ok(models) => {
                added += merge_models(&mut catalog, &mut known_keys, models);
            }
            Err(e) => eprintln!("Mistral model list failed: {e}"),
        }
    }

    // DeepSeek
    if let Ok(api_key) = std::env::var("DEEPSEEK_API_KEY") {
        match fetch_deepseek_models(&client, &api_key).await {
            Ok(models) => {
                added += merge_models(&mut catalog, &mut known_keys, models);
            }
            Err(e) => eprintln!("DeepSeek model list failed: {e}"),
        }
    }

    if added > 0 {
        println!("Added {added} new model(s) to catalog");
    } else {
        println!("Catalog is up-to-date, no new models");
    }

    // Enrich catalog with models.dev metadata
    let enriched = match fetch_models_dev_metadata(&client).await {
        Ok(metadata) => {
            enrich_catalog(&mut catalog, &metadata);
            println!("Enriched catalog with models.dev metadata");
            true
        }
        Err(e) => {
            eprintln!("models.dev metadata fetch failed (non-fatal): {e}");
            false
        }
    };
