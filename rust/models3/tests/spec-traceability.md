...
| 2.5 | Module-level default client behavior | MUST | `tests/spec_2_client.rs` | Covered | 1 test: `set_default_client()` override replaces previous default and is observed by `get_default_client()` |
...
| 7.x | Provider request/response/stream/error translation | MUST | `tests/spec_7_adapters.rs` | Covered | 54 tests across all 6 adapters: OpenAI Responses (request/response/stream/error + provider_options + local image-path -> data URI + missing local image-path error + call_id translation + serialized tool output + thinking portability), Chat Completions (request/response/stream/error + stream-error event + builtin-tools guard via openai + builtin-tools guard via adapter options + thinking portability), Anthropic (request/response/stream/error + thinking signature round-trip + local image-path -> base64 source + auto-cache conversation prefix + provider_options beta_headers + beta_features alias + cache tokens + reasoning token estimation), Gemini (request/response/stream/error + stream-error event + TextEnd on finishReason + thinking + function calls + local image-path -> inlineData + provider_options passthrough + non-object rejection + unknown tool_call_id rejection), Mistral (request basic + null-content omission + provider_options namespace + response basic + stream text-and-finish + error translation), DeepSeek (request basic + provider_options namespace + openai_compatible namespace + builtin-tools rejection + response basic + stream text-and-finish + error translation) |
...
| 8.3 | Message & content model including local image file-path handling | MUST | `tests/spec_7_adapters.rs` + `tests/spec_8_acceptance.rs` | Covered | Adapter tests verify local file paths for OpenAI/Anthropic/Gemini; acceptance keeps cross-provider base64 image parity checks |
| 8.5 | Reasoning token reporting across providers | MUST | `tests/spec_7_adapters.rs` | Covered | OpenAI `reasoning_tokens` mapping, Gemini `thoughtsTokenCount` mapping, Anthropic thinking-token estimation for both complete and stream paths |
| 8.9 | Cross-provider parity matrix | MUST | `tests/spec_8_acceptance.rs` | Covered | 9 tests (env-gated): simple text, streaming text, single tool call, streaming with tools, structured output, usage tokens, image input base64, error invalid key (env-gated per provider), error nonexistent model — all across OpenAI/Anthropic/Gemini/Mistral/DeepSeek (image input excludes DeepSeek — no vision support); provider-scoped cases skip on quota/rate-limit constraints |
| 8.10 | Integration smoke test | MUST | `tests/spec_8_acceptance.rs` | Covered | 5 tests (env-gated): basic generation, streaming, tool calling, structured output, error handling — per spec §8.10 scenarios 1-3, 5-6; provider-scoped cases skip on quota/rate-limit constraints |
| 8.x | Provider options end-to-end | SHOULD | `tests/integration/{openai,anthropic,gemini,mistral,deepseek}.rs` | Covered | 6 tests (env-gated): Anthropic beta_headers, Anthropic auto_cache disabled, Gemini safety settings, OpenAI custom headers, Mistral basic generation, DeepSeek basic generation |
