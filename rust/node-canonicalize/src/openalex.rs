...
use stencila_codec_openalex::{
    OpenAlexWork, search_authors, search_institutions, search_works_title_date, work_by_doi,
};
...
pub(super) async fn reference(reference: &mut Reference) -> Result<()> {
    let work: OpenAlexWork = if let (Some(doi), true) = (&reference.doi, is_doi(&reference.doi)) {
        ...
        let mut works = search_works_title_date(&title, reference.date.clone()).await?;
