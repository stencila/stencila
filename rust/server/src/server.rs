...
/// will use that, otherwise will generate a random server token.
pub fn get_server_token() -> String {
    ...
    // Prefix with `sst_` to make it clear this is a Stencila server token.
    ["sst_", &random].concat()
}
...
async fn auth_middleware(
    ...
    // Check if the server token is provided as an Authorization header
    ...
    // Check if the server token is provided as a cookie
    ...
    // Check if the server token is provided as a query parameter
