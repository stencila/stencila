//! Block-level HTML element decoder for arXiv LaTeXML content.
//!
//! This module handles conversion of HTML block elements (sections, figures, tables,
//! lists, etc.) generated by LaTeXML into Stencila Block nodes. LaTeXML preserves
//! semantic structure through CSS classes like "ltx_section", "ltx_theorem", etc.
...
/// Decode HTML block elements into Stencila Block nodes
///
/// This function recursively processes HTML elements, using tag names and LaTeXML
/// CSS classes to determine the appropriate Stencila Block type. Special handling
/// is provided for:
/// - LaTeX sections and subsections
/// - Mathematical content (equations, theorems, proofs)
/// - Figures and tables with captions
/// - Bibliography sections
/// - Lists with proper numbering cleanup
pub fn decode_blocks(
        ...
        .flat_map(|handle| handle.get(parser))
    {
        if let Some(tag) = child.as_tag() {
            ...
            // Bibliography sections are handled at the article level to avoid duplication
            // when they appear nested within other sections
            ...
            match tag_name.as_ref() {
                "section" => {
                    // LaTeXML marks appendices with ltx_appendix class
                    if tag_class.contains("ltx_appendix") {
                        // Insert appendix break marker only once
...
/// Remove LaTeXML-generated list numbers from list item content
///
/// LaTeXML often includes explicit numbering (e.g., "1. ", "2. ") in list item
/// content, but HTML ordered lists should handle numbering automatically.
/// This function identifies and removes such prefixes.
fn decode_list_item_blocks(
    ...
    // For ordered lists, clean up LaTeXML's explicit numbering
    if ordered && !blocks.is_empty() {
        // Check if first block contains only a list number (e.g., "1.")
        let should_remove_first_block = if let Block::Paragraph(paragraph) = &blocks[0] {
            paragraph.content.len() == 1
                && paragraph
                    .content
                    .first()
                    .and_then(|inline| match inline {
                        Inline::Text(text_node) => Some(&text_node.value),
                        _ => None,
                    })
                    .map(|text| is_list_number_only(text))
                    .unwrap_or(false)
        } else {
            false
        };
        ...
        if should_remove_first_block {
            // Remove paragraph containing only list number
            blocks.remove(0);
        } else {
            // Try to strip number prefix from first text node
                ...
                && let Some(Inline::Text(text_node)) = paragraph.content.first_mut()
            {
                ...
                if let Some(content_start) = find_content_after_list_number(text) {
                    if content_start < text.len() {
                        // Keep content after the list number
                        text_node.value = text[content_start..].to_string().into();
                    } else {
                        // Text was only a number, remove this text node
