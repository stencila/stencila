    ...
    Fut: std::future::Future<Output = Result<Node>> + Send + 'static,
{
    ...
    // Add the specimen route to routes_set so link rewriting recognises it,
    // but filter it from nav items so it does not appear in nav-tree/nav-menu.
    routes_set.insert(crate::specimen::SPECIMEN_ROUTE.to_string());
    nav_items = crate::nav_common::filter_specimen_from_nav(&nav_items);
    ...
    // Render the specimen page (always generated, even during incremental renders)
    ...
    .await
    {
        tracing::warn!("Failed to render specimen page: {}", e);
    }
...
mod tests {
    ...
    // ---- Specimen render integration tests ----
    //
    // These tests exercise the full render() pipeline to verify that the
    // specimen page is wired into the site render process.
    //
    // Note on self-containment: render() calls stencila_config::get() (a
    // process-global OnceLock singleton) to discover routes and load config.
    // We cannot inject a custom SiteConfig into render() directly. Each
    // integration test therefore creates a self-contained temp source
    // directory with a minimal Markdown document and a fresh output dir.
    // The config singleton resolves once from CWD at test-runner startup
    // and the site_config it returns is used by all render() calls.

    /// Helper: call render() on a self-contained temp site and return
    /// `(RenderResult, source_dir_handle, output_dir_handle)`.
    ///
    /// Creates a temp source directory with a single `index.md` so the
    /// render pipeline has at least one route to process. Returns the
    /// TempDir handles so callers can inspect the output.
    async fn render_self_contained_site(
        source_files: Option<&[PathBuf]>,
    ) -> Result<(RenderResult, TempDir, TempDir)> {
        use stencila_codec::DecodeOptions;
        use stencila_codecs::from_str as decode_str;
        use stencila_format::Format as StencilaFormat;

        let source_dir = TempDir::new()?;
        let output_dir = TempDir::new()?;

        // Create a minimal Markdown document so the render pipeline has
        // at least one real route to process.
        fs::write(
            source_dir.path().join("index.md"),
            "# Hello\n\nMinimal page.\n",
        )?;
        ...
        let result = render(
            ...
            source_files,
            None,
            |doc_path, _arguments| async move {
                let content = tokio::fs::read_to_string(&doc_path).await?;
                decode_str(
                    &content,
                    Some(DecodeOptions {
                        format: Some(StencilaFormat::Markdown),
                        ..Default::default()
                    }),
                )
                .await
        ...
        Ok((result, source_dir, output_dir))
    }

    #[tokio::test]
    async fn test_render_produces_specimen_index_html_with_markers() -> Result<()> {
        // AC-1: An integration test calls render() on a minimal site source
        // directory and verifies _specimen/index.html exists in the output
        // and contains expected HTML markers.
        let (_result, _source_dir, output_dir) = render_self_contained_site(None).await?;

        let specimen_path = output_dir.path().join("_specimen/index.html");
        assert!(
            specimen_path.exists(),
            "_specimen/index.html should be generated by the render pipeline"
        );
        ...
        assert!(
            html.contains("id=code"),
            "specimen HTML should contain heading id=code"
        );
        assert!(
            html.contains("id=tables"),
            "specimen HTML should contain heading id=tables"
        );
    ...
    #[tokio::test]
    async fn test_render_specimen_layout_reflects_config() -> Result<()> {
        // AC-2 (integration): Verify that the rendered _specimen/index.html
        // layout reflects the site_config that render() loads from the
        // config singleton.
        //
        // Because render() obtains its SiteConfig via the process-global
        // stencila_config::get() singleton, we cannot inject a custom
        // specimen.layout per-test. Instead we verify that render() does
        // pass the loaded config through to the specimen page:
        //
        //  - Read the config singleton to learn whether specimen.layout
        //    or site.layout is configured.
        //  - Render via the full pipeline.
        //  - Assert that the rendered _specimen/index.html contains a
        //    <stencila-layout> wrapper (proving layout was applied).
        //  - If a specimen or site layout is configured, check that the
        //    rendered HTML is structurally consistent with it.
        //
        // This proves the render() wiring passes config through. The
        // helper-level tests below cover the detailed override semantics
        // with controlled SiteConfig values.

        let loaded_config = stencila_config::get()?;
        let site_config = loaded_config.site.as_ref().cloned().unwrap_or_default();
        ...
        // The specimen page must always have a layout wrapper regardless
        // of which layout config is active.
        assert!(
            html.contains("<stencila-layout"),
            "specimen HTML should contain the <stencila-layout> wrapper"
        );

        // If the config has a specimen-specific layout with left-sidebar
        // disabled, the rendered output must not contain a left-sidebar.
        // If the config uses a default layout or no layout, this section
        // verifies consistency.
        let effective_layout = site_config
            .specimen
            .as_ref()
            .and_then(|s| s.layout.as_ref())
            .or(site_config.layout.as_ref());

        if let Some(layout) = effective_layout
            && let Some(ref left) = layout.left_sidebar
            && !left.is_enabled()
        {
            assert!(
                !html.contains("<stencila-left-sidebar"),
                "specimen HTML should not contain left-sidebar when config disables it"
            );
        }
    ...
    #[tokio::test]
    async fn test_render_specimen_regenerated_on_incremental_render() -> Result<()> {
        // AC: Incremental re-render (with source_files filter) should still
        // regenerate the specimen page because it has no source file — it
        // must always be produced.

        // Full render first — specimen should be generated
        let (_result, source_dir, output_dir) = render_self_contained_site(None).await?;

        let specimen_path = output_dir.path().join("_specimen/index.html");
        assert!(
            specimen_path.exists(),
            "specimen should exist after full render"
        );

        // Delete the specimen file to simulate a dirty state
        fs::remove_file(&specimen_path)?;
        assert!(!specimen_path.exists());

        // Incremental render: pass a source file to trigger the
        // source_files.is_some() branch in render(). The specimen page
        // should be regenerated regardless.
        let index_path = source_dir.path().join("index.md");
        ...
        assert!(
            specimen_path.exists(),
            "specimen page should be regenerated even during incremental render with source_files filter"
        );

        Ok(())
    }

    // ---- Specimen helper-level tests ----
    //
    // These test render_specimen_page() directly to verify layout config
    // behavior that is controlled by function parameters. They complement
    // the integration tests above by testing override semantics with
    // controlled SiteConfig values.

    #[tokio::test]
    async fn test_specimen_page_applies_specimen_layout_override() -> Result<()> {
        use crate::specimen::render_specimen_page;
        use std::collections::HashSet;
        use stencila_config::{LayoutConfig, LayoutPreset, RegionSpec, SpecimenConfig};

        let output_dir = TempDir::new()?;

        // Configure a specimen-specific layout with left-sidebar disabled
        let site_config = SiteConfig {
            specimen: Some(SpecimenConfig {
        ...
        // AC-2: The rendered HTML should NOT contain a left-sidebar element
        // because the specimen layout config disabled it
        assert!(
            !html.contains("<stencila-left-sidebar>"),
            "specimen HTML should not contain left-sidebar when disabled in specimen layout config"
        );

        // But it should still have a layout wrapper
        assert!(
            html.contains("<stencila-layout"),
            "specimen HTML should contain the layout wrapper"
        );
    ...
    #[tokio::test]
    async fn test_specimen_page_uses_default_layout_when_no_specimen_config() -> Result<()> {
        use crate::specimen::render_specimen_page;
        use std::collections::HashSet;
        use stencila_config::{LayoutConfig, LayoutPreset};

        let output_dir = TempDir::new()?;

        // Site has a layout but NO specimen-specific config
        let site_config = SiteConfig {
            layout: Some(LayoutConfig {
                preset: Some(LayoutPreset::Docs),
                ..Default::default()
        ...
        // AC-3: When no specimen config is set, the site's default layout should apply.
        // The docs preset enables left-sidebar by default.
        assert!(
            html.contains("<stencila-left-sidebar>"),
            "specimen HTML should contain left-sidebar from site's default docs layout"
        );

        assert!(
            html.contains("<stencila-layout"),
            "specimen HTML should contain the layout wrapper from site's default layout"
        );
