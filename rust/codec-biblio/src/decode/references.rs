...
pub fn reference(input: &str) -> Reference {
    ...
    match parser.parse(input) {
        ...
        Err(error) => {
            ...
            if use_partial {
                parser.parse(input).unwrap_or_else(|_| fallback(input))
            } else {
                fallback(input)
            }
        }
...
mod tests {
    ...
    fn zenodo_15308198() {
        let r = reference(
            "1.\tBirla, N. (2019). Vehicle Dataset from CarDekho. Retrieved from: https://www.kaggle.com/datasets/nehalbirla/vehicle-dataset-from-cardekho",
        );
        assert_eq!(r.work_type, Some(CreativeWorkType::WebPage));
        assert_eq!(
            r.url,
    ...
    #[test]
    fn arxiv_2507_09057v1() {
        let r = reference(
            "American Dental Association (2022). Eruption Charts. https://www.mouthhealthy.org/all-topics-a-z/eruption-charts.",
        );
        assert_eq!(r.work_type, Some(CreativeWorkType::WebPage));
        assert_eq!(
            r.url,
    ...
    fn arxiv_2507_11127v1() {
        ...
        let r = reference(
            "Vaishak Belle, Andrea Passerini, Guy Van den Broeck, et al. Probabilistic inference in hybrid domains by weighted model integration. In Proceedings of 24th International Joint Conference on Artificial Intelligence (IJCAI), pages 2770–2776. AAAI Press/International Joint Conferences on Artificial Intelligence, 2015.",
        );
        assert_eq!(r.work_type, Some(CreativeWorkType::Article));
        assert_eq!(
            r.title.map(|title| to_text(&title)),
    ...
    #[test]
    fn arxiv_2507_11353v1() {
        let r = reference(
            "Acharya, V. V., Berner, R., Engle, R., Jung, H., Stroebel, J., Zeng, X. and Zhao, Y. (2023), ‘Climate stress testing’, Annual Review of Financial Economics 15(1), 291–326.",
        );
        assert_eq!(r.work_type, Some(CreativeWorkType::Article));
        assert_eq!(
            r.is_part_of
    ...
    #[test]
    fn biorxiv_2024_12_10_627863v2() {
        // "et al" without a preceding comma was unrecognized
        let r = reference(
            r#"S. M. Paul et al., "How to improve R&D productivity: The pharmaceutical industry's grand challenge," Nat Rev Drug Discov, vol. 9, no. 3, pp. 203-214, 2010."#,
        );
        assert_eq!(r.work_type, Some(CreativeWorkType::Article));
        assert_eq!(
            r.is_part_of
        ...
        let r = reference(
            r#"A. P. Davis, T. C. Wiegers, R. J. Johnson, D. Sciaky, J. Wiegers, and C. J. Mattingly, "Comparative Toxicogenomics database (CTD): Update 2023," Nucleic Acids Res, vol. 51, no. D1, pp. D1257-D1262, 2023"#,
        );
        assert_eq!(r.work_type, Some(CreativeWorkType::Article));
        assert_eq!(
            r.is_part_of
    ...
    #[test]
    fn biorxiv_2025_05_12_25325848v3() {
        // No space between initials, and abbreviated journal name
        let r = reference(
            r#"Pfohl, S.R., Kim, R.B., Coan, G.S., and Mitchell, C.S. (2018). Unraveling the complexity of amyotrophic lateral sclerosis survival prediction. Front. Neuroinform. 12"#,
        );
        assert_eq!(r.work_type, Some(CreativeWorkType::Article));
        assert!(r.date.is_some());
        assert_eq!(
        ...
        let r = reference(
            r#"Beghi, E., Chiò, A., Couratier, P., Esteban, J., Hardiman, O., Logroscino, G., Millul, A., Mitchell, D., Preux, P.-M., Pupillo, E., et al. (2011). The epidemiology and treatment of ALS: focus on the heterogeneity of the disease and critical appraisal of therapeutic trials. Amyotroph. Lateral Scler. 12, 1-10"#,
        );
        assert_eq!(r.work_type, Some(CreativeWorkType::Article));
        assert!(r.date.is_some());
        assert_eq!(
