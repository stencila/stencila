...
use codec::{
    ...
    schema::{Author, Citation, CitationMode, CitationOptions, Person, shortcuts::t},
};
...
fn author_year_parenthetical(input: &mut &str) -> Result<Citation> {
    (
        authors,
        alt(((multispace0, ",", multispace0).take(), multispace1)),
        year_az,
...
mod tests {
    ...
    #[test]
    fn test_author_year_parenthetical() -> Result<()> {
        // Single author variations
        let citation = author_year_parenthetical(&mut "Smith 1990")?;
        ...
        let citation = author_year_parenthetical(&mut "Jones 2023a")?;
        assert_eq!(citation.target, "jones-2023a");

        let citation = author_year_parenthetical(&mut "Jones, 2023z")?;
        assert_eq!(citation.target, "jones-2023z");

        // Multi-part family name
        let citation = author_year_parenthetical(&mut "Van Der Berg 2017")?;
        ...
        let citation = author_year_parenthetical(&mut "Smith-Jones 2016")?;
        assert_eq!(citation.target, "smith-jones-2016");

        let citation = author_year_parenthetical(&mut "O'Connor, 2015")?;
        assert_eq!(citation.target, "o-connor-2015");

        // Two authors with ampersand
        let citation = author_year_parenthetical(&mut "Smith & Jones 1995")?;
        ...
        let citation = author_year_parenthetical(&mut "Taylor and Wilson 2020")?;
        ...
        // Et al variations
        let citation = author_year_parenthetical(&mut "Johnson et al. 2019")?;
        assert_eq!(citation.target, "johnson-et-al-2019");

        let citation = author_year_parenthetical(&mut "Johnson et al., 2019")?;
        assert_eq!(citation.target, "johnson-et-al-2019");

        let citation = author_year_parenthetical(&mut "Garcia et al 2018")?;
        ...
        let citation = author_year_parenthetical(&mut "Brown 1200")?;
        assert_eq!(citation.target, "brown-1200");

        let citation = author_year_parenthetical(&mut "Miller 2050")?;
        assert_eq!(citation.target, "miller-2050");

        // Complex multi-part names
        let citation = author_year_parenthetical(&mut "Von Der Leyen & Garcia-Martinez 2021")?;
        assert_eq!(citation.target, "von-der-leyen-and-garcia-martinez-2021");

        // Non-matches - invalid years
        assert!(author_year_parenthetical(&mut "Smith 1199").is_err()); // Year too early
        assert!(author_year_parenthetical(&mut "Smith 2051").is_err()); // Year too late
        assert!(author_year_parenthetical(&mut "Smith 999").is_err()); // Year too short

        // Non-matches - missing components
        assert!(author_year_parenthetical(&mut "Smith").is_err()); // No year
        assert!(author_year_parenthetical(&mut "1990").is_err()); // No author
        assert!(author_year_parenthetical(&mut "").is_err()); // Empty string

        // Valid multi-word family name (actually matches as single author)
        let citation = author_year_parenthetical(&mut "Smith Jones 1990")?;
        assert_eq!(citation.target, "smith-jones-1990");

        // Non-matches - invalid formats
        assert!(author_year_parenthetical(&mut "Smith & 1990").is_err()); // Missing second author
        assert!(author_year_parenthetical(&mut "& Jones 1990").is_err()); // Missing first author
        assert!(author_year_parenthetical(&mut "Smith et 1990").is_err()); // Incomplete "et al"
        assert!(author_year_parenthetical(&mut "Smith al 1990").is_err()); // Missing "et"

        // Non-matches - invalid characters
        assert!(author_year_parenthetical(&mut "Sm1th 1990").is_err()); // Numbers in name
        assert!(author_year_parenthetical(&mut "Smith@ 1990").is_err()); // Special characters in name
        assert!(author_year_parenthetical(&mut "Smith 199a").is_err()); // Letters in year (except suffix)
