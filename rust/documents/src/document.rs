...
/// The instant the document was last written
pub type DocumentLastWritten = Arc<RwLock<Instant>>;
...
pub type DocumentPatchRequestSender = mpsc::Sender<PatchRequest>;
pub type DocumentPatchRequestReceiver = mpsc::Receiver<PatchRequest>;
...
pub type DocumentWriteRequestSender = mpsc::Sender<WriteRequest>;
pub type DocumentWriteRequestReceiver = mpsc::Receiver<WriteRequest>;
...
impl Document {
    ...
    fn new(path: Option<PathBuf>, format: Option<String>) -> Result<Document> {
        ...
        let last_written = Arc::new(RwLock::new(Instant::now()));
    ...
    pub async fn write(&mut self, content: Option<String>, format: Option<String>) -> Result<()> {
        ...
        self.status = DocumentStatus::Synced;
        *self.last_written.write().await = Instant::now();
    ...
    async fn modified(&mut self, path: PathBuf) {
        if self.last_written.read().await.elapsed() < Duration::from_millis(Self::WRITE_MUTE_MILLIS)
        {
            return;
        }
