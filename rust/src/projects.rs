...
/// The type of build for a project
...
impl Project {
    ...
    /// Identify the project by giving it an `id` if it does not yet have one.
    ///
    /// If the project does not have an `id`, then it will be assigned one and
    /// the project written to disk so that the `id` is persisted.
    ///
    /// The id is random sample of 24 characters from the lowercase ASCII letters and digits.
    /// About 77 billion years would be needed, in order to have a 1% probability of at least one
    /// collision assuming 1000 ids are generated per hour (https://zelark.github.io/nano-id-cc/).
    /// Uppercase letters are not used because Docker image names can not contain them.
    ...
    /// Build the project
    pub async fn build(&mut self, build: ProjectBuild) -> Result<()> {
        match build {
            ProjectBuild::Docker => self.build_docker().await?,
        }
        Ok(())
    }
    ...
    ///
    /// Builds a Docker image from a `Dockerfile` in the project's root folder (generated if
    /// necessary). Uses Docker to build the image, if available, falling back to using Buildah,
    /// if available.
    ///
    /// The image will be tagged with the name "stencila/project-<id>" where "<id>" is the
    /// unique identifier for the project.
    pub async fn build_docker(&mut self) -> Result<()> {
        let dockerfile = self.build_dockerfile(None)?;

        let id = self.identify().await?;
        let tag = format!("stencila/project-{}", id);

        // We tried to use `bollard` to build images but it is quite complicated requiring us to
        // build a tar ball, including having to exclude `dockerignore` files and stream it to the
        // Docker daemon. It also requires additional dependencies. For simplicity, we use the
        // Docker CLI client which is part of the Docker install on all platforms.
        //
        // See these examples if attempting again:
        //   https://github.com/fussybeaver/bollard/blob/master/examples/image_from_scratch.rs
        //   https://github.com/fussybeaver/bollard/blob/master/examples/post_dockerfile.rs
        #[cfg(feature = "binaries")]
        if let Ok(docker) = crate::binaries::require("docker", "*").await {
            tracing::debug!("Building image using Docker");

            docker
                .run_async(&[
                    "build".to_string(),
                    format!("--tag={}", tag),
                    self.path.to_slash().expect("Should convert to slash path"),
                ])
                .await?;
        ...
        #[cfg(feature = "binaries")]
        if let Ok(buildah) = crate::binaries::require("buildah", "*").await {
            tracing::debug!("Building image using Buildah");

            buildah
                .run_async(&[
                    "bud".to_string(),
                    format!("--file={}", dockerfile.display()),
                    format!("--tag={}", tag),
                ])
                .await?;
        ...
        bail!("Unable to build Docker image: neither Docker, nor Buildah, appear to be installed")
    }

    /// Build a Dockerfile for the project
    ///
    /// If the project does not have a `Dockerfile`, or the existing `Dockerfile`
    /// has the "Generated by Stencila" header, then a `Dockerfile` will be created.
    /// Otherwise, does nothing.
    ///
    /// # Arguments
    ///
    /// - `from`: The base image used in the `FROM` instruction of the `Dockerfile`.
    ///           Currently defaults to `scratch` but likely to be `stencila/project-base`
    ///           in the future.
    pub fn build_dockerfile(&self, from: Option<String>) -> Result<PathBuf> {
        let dockerfile = self.path.join("Dockerfile");

        if dockerfile.exists() {
            let content = fs::read_to_string(&dockerfile)?;
            if !content.starts_with("# Generated by Stencila") {
        ...
        let content = format!(
            r#"
# Generated by Stencila
# Please remove this comment block if you wish to make edits\n
# and stop Stencila from regenerating this file.

FROM {from}
"#,
            from = from.unwrap_or("scratch".to_string())
        );
        fs::write(&dockerfile, content.trim())?;
...
pub mod cli {
    ...
    impl Build {
        pub async fn run(self) -> display::Result {
            let Self { folder } = self;
            let mut project = PROJECTS.open(folder, false).await?;
            project.build(ProjectBuild::Docker).await?;
            display::nothing()
        }
    }
