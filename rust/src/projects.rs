...
impl Project {
    ...
    ///
    /// First builds a `Dockerfile` for the project (if none exists).
    /// Then builds a Docker image from the `Dockerfile` file using Docker, if available,
    /// falling back to using Buildah, if available. The image will be tagged with the
    /// name "stencila/project-<id>" where "<id>" is the unique identifier for the project.
    pub async fn build_docker(&mut self) -> Result<()> {
        use futures::StreamExt;

        let dockerfile = self.build_dockerfile()?;
        ...
        if docker.ping().await.is_ok() {
            ...
            while let Some(item) = stream.next().await {
                match &item {
                    Ok(info) => tracing::info!("docker: {:?}", info),
        ...
        if let Ok(buildah) = binaries::require("buildah", "*").await {
    ...
    pub fn build_dockerfile(&self) -> Result<PathBuf> {
...
FROM {from}
"#,
            from = "scratch" // "stencila/executa-all"
        );
