...
/// Valid output command values
const VALID_COMMANDS: &[&str] = &["render", "convert", "none"];
...
fn validate_output_options(
    ...
    if source.is_some() && pattern.is_some() {
        return Err(eyre!(
            "Cannot specify both --source and --pattern for output `{}`",
            key
        ));
    }
    ...
    if pattern.is_some() && !key.contains("*.") {
        return Err(eyre!(
            "Output `{}` with --pattern must include `*.ext` suffix (e.g., 'reports/*.pdf')",
            key
        ));
    }
    ...
    {
        return Err(eyre!(
            "Invalid command `{}` for output `{}`. Valid values: render, convert, none",
            cmd,
            key
        ));
    }
...
pub fn config_add_output(
    ...
    validate_output_options(key, source, command, refs, pattern, exclude)?;
    ...
    if needs_config {
        ...
        if let Some(cmd) = command {
            config_table.insert("command", cmd.into());
        }
