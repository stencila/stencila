...
mod tests {
    ...
    async fn basics() -> Result<()> {
        ...
        // Syntax error
        let (outputs, messages) = kernel
            .exec("bad ^ # syntax", Format::TypeScript, None)
            .await?;
        ...
        // Runtime error
        let (outputs, messages) = kernel.exec("foo", Format::TypeScript, None).await?;
        ...
        // Use both variables
        let (outputs, messages) = kernel.exec("a*b", Format::TypeScript, None).await?;
        assert_json_eq!(messages, json!([]));
    ...
    async fn console_log() -> Result<()> {
        ...
        let (outputs, messages) = kernel
            .exec("console.log(1)", Format::TypeScript, None)
            .await?;
        assert_json_eq!(messages, json!([]));
        assert_json_eq!(outputs, [1]);

        let (outputs, messages) = kernel
            .exec("console.log(1, 2, 3, 4)", Format::TypeScript, None)
            .await?;
        assert_json_eq!(messages, json!([]));
        assert_json_eq!(outputs, [1, 2, 3, 4]);

        let (outputs, messages) = kernel
            .exec("console.log([1, 2, 3], 4, 'str')", Format::TypeScript, None)
            .await?;
        assert_json_eq!(messages, json!([]));
