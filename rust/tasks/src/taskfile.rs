...
impl Taskfile {
    ...
    pub async fn update(&mut self, detect: bool) -> Result<()> {
        ...
        if !build_cmds.is_empty() && !self.tasks.contains_key("build") {
            ...
            default_cmds.push(Command::task("build"))
        }
        ...
        if !self.tasks.contains_key("default") {
            ...
            self.tasks.insert(
                "default".to_string(),
                Task {
                    desc: Some("Default task".to_string()),
                    summary: Some(
                        "Autogenerated task which runs a server and, when it stops e.g. due to inactivity, saves the machine image.".to_string(),
                    ),
                    autogen: true,
                    cmds: default_cmds,
                    ..Default::default()
...
impl Command {
    /// Create a new command with only the `cmd` attribute
    fn cmd(cmd: &str) -> Self {
        Self {
