    ...
    for mut inline in input.into_iter() {
        if let Inline::Citation(current) = &mut inline {
                        ...
                        if let (Ok(start), Ok(end)) = (
                            ...
                            if end > start {
                                ...
                                let mut items = (start..=end)
                                    .map(|target| Citation {
                                        target: [target_prefix.clone(), target.to_string()]
                                            .concat(),
            ...
            if matches!(output.last(), Some(Inline::Citation(..))) {
                if let Some(Inline::Citation(mut previous)) = output.pop() {
            ...
            if let Some(Inline::Text(Text { value, .. })) = output.last() {
                if value.trim() == "," {
                    if let Some(Inline::Citation(citation)) = output.iter().rev().nth(1).cloned() {
