...
pub struct Cli {
    ...
    #[arg(
        long,
        default_value = "globset=warn,hyper=info,hyper_util=info,ignore=warn,keyring=info,mio=info,notify=warn,ort=error,reqwest=info,rustls=info,sled=info,tokenizers:=info,tokio=info,tungstenite=info",
        global = true,
        hide = true
    )]
