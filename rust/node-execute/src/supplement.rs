...
impl Executable for Supplement {
    ...
    async fn compile(&mut self, executor: &mut Executor) -> WalkControl {
        ...
        // Auto-generate an id from the label if the supplement doesn't already
        // have a user-supplied id
        if let Some(label) = &self.label
            && let Some(id) = Executor::auto_id(&LabelType::SupplementLabel, label, &self.id)
        {
        ...
        if let (Some(id), Some(label)) = (&self.id, &self.label) {
            executor.labels.insert(
                id.clone(),
                LabelTarget::from_schema_label_type(LabelType::SupplementLabel, label.clone()),
            );
        }
