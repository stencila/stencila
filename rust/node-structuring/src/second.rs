...
use stencila_schema::{
    Block, Emphasis, Heading, Inline,
    InlinesBlock, Node, Paragraph, Strikeout, Strong, StyledInline,
    Subscript, Superscript, Underline, VisitorMut, WalkControl,
};

use crate::{FirstWalk, StructuringOptions};

/// Second structuring walk
///
/// Walks over a node and uses information collected during the first walk to
/// perform a second round of structuring focussed on replacing text segments
/// with [`Citation`] nodes.
...
impl SecondWalk {
    ...
    fn visit_inlines(&mut self, inlines: &mut Vec<Inline>) {
        ...
        // Second pass to apply any further structuring, including within replacements
        // from the first pass
