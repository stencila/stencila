...
impl SessionConfig {
    ...
    pub async fn from_agent(agent: &crate::definition::AgentInstance) -> eyre::Result<Self> {
...
// ---------------------------------------------------------------------------
// Abort types (shared across API and CLI sessions)
// ---------------------------------------------------------------------------
...
impl AbortController {
    ...
    /// session alive for subsequent submits.
    ///
...
impl AbortSignal {
    ...
    ///
    /// Called automatically at the start of each submit so that a
    /// soft-aborted session is ready for the next exchange.
