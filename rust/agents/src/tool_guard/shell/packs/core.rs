...
pub static FILESYSTEM_PACK: Pack = Pack {
    ...
    destructive_patterns: &[
        destructive_pattern!(
            ...
            r"\brm\b",
            rm_recursive_root_validator,
            "Recursive deletion of root, home, or system directory",
            "Specify the exact subdirectory to delete",
            Confidence::High
        ),
        destructive_pattern!(
            "recursive_delete_force",
            r"\brm\b",
            rm_recursive_force_validator,
            "Forced recursive deletion can destroy entire directory trees without confirmation",
            "Delete specific files by name, or remove the `-f` flag and specify exact paths",
            Confidence::High
        ),
        destructive_pattern!(
            "recursive_delete",
            r"\brm\b",
            rm_recursive_validator,
            "Recursive deletion can destroy directory trees",
            "Delete specific files by name, or list directory contents first with `ls -la`",
            Confidence::Medium
        ),
        destructive_pattern!(
            "find_destructive",
            r"\bfind\b",
            find_destructive_validator,
            "`find` with action flags can delete or execute arbitrary commands on matched files",
            "Use `find` to list matching files first (`find . -name '*.tmp'`), review the output, then delete specific files by name",
            Confidence::Medium
        ),
        destructive_pattern!(
            "mv_system_path",
            r"\bmv\b",
            mv_system_path_validator,
            "Moving system directories can break the OS",
            "Move specific files within subdirectories instead",
            Confidence::Medium
        ),
        destructive_pattern!(
            "shred_device",
            r"\bshred\b",
            shred_device_validator,
            "Shredding devices causes permanent data loss",
        ...
        destructive_pattern!(
            "chmod_broad",
            r"\bchmod\b",
            chmod_broad_validator,
            "Recursive or overly permissive chmod can break system security",
            "Set specific permissions on specific files (e.g., `chmod 644 file`)",
            Confidence::Medium
        ),
        destructive_pattern!(
            "chown_recursive",
            r"\bchown\b",
            chown_recursive_validator,
            "Recursive chown on system directories can break the OS",
            "Change ownership of specific files only",
            Confidence::Medium
        ),
        destructive_pattern!(
            "overwrite_truncate",
            r"(?:>\s*/(?:etc|boot|usr|bin|sbin|lib|proc|sys|dev)/|\btruncate\b[^|><]*/(?:etc|boot|usr|bin|sbin|lib|proc|sys|dev)/)",
            "Overwriting or appending to system files can break the OS",
            "Use `sudo tee` with explicit paths, or edit specific config files",
            Confidence::Medium
        ),
        destructive_pattern!(
            "sensitive_read",
            r"\b(?:cat|bat|head|tail|less|more|strings|xxd)\b",
            sensitive_read_validator,
            "Reading sensitive files (credentials, private keys, auth tokens) can leak secrets into the agent's context window",
            "Use targeted commands that don't expose raw secrets (e.g., `ssh-keygen -l -f ~/.ssh/id_rsa` to check a key fingerprint)",
            Confidence::Medium
        ),
    ],
};
...
pub static GIT_PACK: Pack = Pack {
    ...
    destructive_patterns: &[
        destructive_pattern!(
            ...
            "Use `git stash` to save changes, or `git reset --soft` to unstage",
            Confidence::High
        ),
        destructive_pattern!(
            "force_push",
            r"\bgit\s+push\b",
            force_push_validator,
            "Force push overwrites remote history, potentially losing others' work",
        ...
        destructive_pattern!(
            "checkout_discard",
            r"\bgit\s+checkout\b",
            checkout_discard_validator,
            "Discards uncommitted changes across a broad scope",
            "Checkout specific files: `git checkout -- path/to/file`",
            Confidence::High
        ),
        destructive_pattern!(
            "restore_discard",
            r"\bgit\s+restore\b",
            restore_discard_validator,
            "Discards uncommitted or staged changes across a broad scope",
            "Restore specific files: `git restore path/to/file`",
            Confidence::High
        ),
        destructive_pattern!(
            "rebase_active",
            r"\bgit\s+rebase\b",
            rebase_active_validator,
            "Rebase with `--onto` or `--root` rewrites commit history and can cause data loss",
            "Use `git log` to review history first; simple `git rebase <branch>` is permitted",
            Confidence::Medium
        ),
        destructive_pattern!(
            "branch_force_delete",
            ...
            "Use `git branch -d` which only deletes if fully merged",
            Confidence::Medium
        ),
        destructive_pattern!(
            "stash_drop_clear",
            ...
            "Verify stash contents with `git stash show` before dropping",
            Confidence::Medium
        ),
        destructive_pattern!(
            "worktree_force_remove",
            r"\bgit\s+worktree\s+remove\b.*(?:--force|-f)\b",
            "Force-removes a worktree without checking for uncommitted changes",
            "Use `git worktree remove` without `--force` which checks for uncommitted changes first",
            Confidence::Medium
        ),
        destructive_pattern!(
            "reflog_expire",
            r"\bgit\s+reflog\s+expire\b",
            reflog_expire_validator,
            "Expiring reflog entries permanently destroys the ability to recover previous HEAD positions",
            "Use `git reflog` to inspect entries before expiring; avoid `--expire=now`",
            Confidence::High
        ),
        destructive_pattern!(
            "gc_prune",
            r"\bgit\s+gc\b",
            gc_prune_validator,
            "Aggressive garbage collection permanently removes unreachable objects",
            "Use `git gc` without `--prune=now` which uses a safe 2-week grace period",
            Confidence::Medium
        ),
    ],
};
...
pub static OBFUSCATION_PACK: Pack = Pack {
    ...
    destructive_patterns: &[
        destructive_pattern!(
            "pipe_to_shell",
            r"\|\s*(?:bash|sh|zsh|dash|fish|csh|tcsh|ksh)\b",
            "Piping content to a shell interpreter bypasses command inspection",
            "Download/generate the script to a file, review it, then execute",
            Confidence::High
        ),
        destructive_pattern!(
            "base64_to_shell",
            r"\bbase64\b.*\|\s*(?:bash|sh|zsh|dash|fish|csh|tcsh|ksh|eval)\b",
            "Base64-encoded commands bypass pattern matching",
            "Write the command directly instead of encoding it",
            Confidence::High
        ),
        destructive_pattern!(
            "eval_subshell",
            r"\beval\s+(?:\$\(|`)",
            "eval of dynamic content bypasses command inspection",
            "Execute the inner command directly",
            Confidence::High
        ),
        destructive_pattern!(
            "curl_pipe_shell",
            r"\b(?:curl|wget)\b.*\|\s*(?:bash|sh|zsh|dash|fish|csh|tcsh|ksh|eval)\b",
            "Downloading and executing untrusted code in one step",
            "Download the script first with `curl -o script.sh`, review it, then execute",
            Confidence::High
        ),
        destructive_pattern!(
            "python_exec",
            r"\bpython[23]?\s+-c\b[^|><]*\b(?:os\.(?:system|popen|execl|execle|execlp|execlpe|execv|execve|execvp|execvpe)|subprocess\.(?:run|call|Popen))\b",
            "Python used as a shell wrapper to bypass command inspection",
...
pub static STENCILA_PACK: Pack = Pack {
    ...
    destructive_patterns: &[
        destructive_pattern!(
            "secrets_modify",
            r"\bstencila\s+secrets\s+(?:set|delete|remove)\b",
            "Modifying secrets can break authentication",
            "Use `stencila secrets list` to view secrets",
            Confidence::High
        ),
        destructive_pattern!(
            "auth_modify",
            r"\bstencila\s+auth\s+(?:login|logout)\b",
            "Changing authentication state affects all sessions",
            "Use `stencila auth status` to check auth state",
            Confidence::High
        ),
        destructive_pattern!(
            "cloud_auth",
            r"\bstencila\s+cloud\s+(?:signin|signout)\b",
            "Changing cloud authentication affects deployments",
            "Use `stencila cloud status` to check cloud state",
            Confidence::High
        ),
        destructive_pattern!(
            "publish",
            r"\bstencila\s+publish\b",
            "Publishing makes content publicly visible",
            "Review content manually before publishing",
            Confidence::High
        ),
        destructive_pattern!(
            "uninstall",
            r"\bstencila\s+uninstall\b",
            "Uninstalling removes the runtime",
            "This operation should not be performed by an agent",
            Confidence::High
        ),
        destructive_pattern!(
            "push",
            r"\bstencila\s+push\b",
            "Pushing sends local changes to remote",
            "Use `stencila status` to review changes first",
            Confidence::Medium
        ),
        destructive_pattern!(
            "db_destructive",
            r"\bstencila\s+db\s+(?:reset|gc)\b",
            "Database reset/gc can lose data",
            "Use `stencila db status` to inspect the database first",
            Confidence::Medium
        ),
        destructive_pattern!(
            "clean",
            r"\bstencila\s+clean\b",
            "Clean removes generated artifacts",
            "Use `stencila status` to see what will be removed",
            Confidence::Medium
        ),
    ],
};
