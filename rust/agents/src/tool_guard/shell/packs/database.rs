...
pub static POSTGRESQL_PACK: Pack = Pack {
    ...
    destructive_patterns: &[
        destructive_pattern!("drop_database", r"(?i)\bDROP\s+DATABASE\b", "Permanently destroys the entire database", "Use `pg_dump` to backup first", Confidence::High),
        destructive_pattern!("drop_table", r"(?i)\bDROP\s+TABLE\b", "Permanently destroys a table and all its data", "Use `pg_dump -t <table>` to backup first; use a transaction with `BEGIN`/`ROLLBACK` for safety", Confidence::High),
        destructive_pattern!("truncate", r"(?i)\bTRUNCATE\s+(?:TABLE\s+)?\w", "Removes all rows without logging individual deletions", "Use `DELETE FROM` with a `WHERE` clause for selective deletion", Confidence::Medium),
        destructive_pattern!("delete_no_where", r"(?i)\bDELETE\s+FROM\b", delete_no_where_validator, "Deletes all rows from a table", "Add a `WHERE` clause to limit deletion scope. If your query already has a WHERE clause on a separate line, combine them onto one line (e.g., `DELETE FROM users WHERE active = false`)", Confidence::Medium),
    ],
};
...
pub static MYSQL_PACK: Pack = Pack {
    ...
    safe_patterns: &[],
    destructive_patterns: &[
        destructive_pattern!("drop_database", r"(?i)\bDROP\s+(?:DATABASE|SCHEMA)\b", "Permanently destroys the entire database", "Use `mysqldump` to backup first", Confidence::High),
        destructive_pattern!("drop_table", r"(?i)\bDROP\s+TABLE\b", "Permanently destroys a table and all its data", "Backup the table first with `mysqldump`", Confidence::High),
        destructive_pattern!("truncate", r"(?i)\bTRUNCATE\s+(?:TABLE\s+)?\w", "Removes all rows without logging", "Use `DELETE FROM` with a `WHERE` clause", Confidence::Medium),
        destructive_pattern!("delete_no_where", r"(?i)\bDELETE\s+FROM\b", delete_no_where_validator, "Deletes all rows from a table", "Add a `WHERE` clause. If your query already has a WHERE clause on a separate line, combine onto one line", Confidence::Medium),
    ],
};
...
pub static SQLITE_PACK: Pack = Pack {
    ...
    safe_patterns: &[],
    destructive_patterns: &[
        destructive_pattern!("drop_table", r"(?i)\bDROP\s+TABLE\b", "Permanently destroys a table", "Backup the database file first", Confidence::High),
        destructive_pattern!("delete_no_where", r"(?i)\bDELETE\s+FROM\b", delete_no_where_validator, "Deletes all rows from a table", "Add a `WHERE` clause. If your query already has a WHERE clause on a separate line, combine onto one line", Confidence::Medium),
    ],
};
...
mod tests {
    ...
    fn delete_no_where_matches() {
        ...
        assert!(!delete_no_where_validator("delete from users where active = false"));
        // Quoted SQL in shell commands
        assert!(delete_no_where_validator("psql -c 'DELETE FROM users'"));
        assert!(!delete_no_where_validator("psql -c 'DELETE FROM users WHERE id = 1'"));
        assert!(delete_no_where_validator(r#"sqlite3 db.sqlite "DELETE FROM users;""#));
        assert!(!delete_no_where_validator(r#"sqlite3 db.sqlite "DELETE FROM users WHERE id = 1;""#));
    }
