...
You _can_ set an explicit label by placing it after the `figure` keyword. However, it is almost always better to omit explicit labels and let Stencila assign them automatically. Explicit labels do not update when you rearrange your document and can easily get out of sync.
...
| Intent | Layout | Description |
| --- | --- | --- |
| Single column | _(none)_ | Default — subfigures stack vertically in a single column |
...
| Layout | Meaning |
| --- | --- |
| `a b` | Two subfigures side-by-side |
| `a b \| a c` | `a` spans two rows on the left; `b` and `c` stack on the right |
| `a a \| b c` | `a` spans two columns on top; `b` and `c` sit below |
| `a . \| b c` | Top-right cell is empty |
...
For layouts that need both unequal column widths _and_ precise placement, put the column specification before a `:` and the layout map after it:
...
Overlay shapes and text inherit theme defaults — stroke color, stroke width, fill, font family, font size, and font weight — from the `--figure-overlay-*` [design tokens](../themes/tokens/nodes/figures). In common cases you only need to specify geometry; when you need a specific color, weight, dash pattern, or other non-default treatment on an element, set it explicitly as an inline SVG attribute and it will take precedence over the theme default.

The examples below use [overlay components](figure-overlays) — high-level elements like `<s:callout>`, `<s:arrow>`, `<s:scale-bar>`, and `<s:roi-rect>` that expand to standard SVG during compilation. Components handle arrowhead definitions, label positioning, and coordinate math automatically. You can also write raw SVG directly; see the [overlay components reference](figure-overlays) for the full component catalog and details on mixing components with raw SVG.
...
A figure with several overlay annotations: a [`<s:halo>`](figure-overlays#halo) and [`<s:callout>`](figure-overlays#callout) highlighting the center, a dashed [`<s:roi-rect>`](figure-overlays#roi-rectangle) bounding box, and an [`<s:arrow>`](figure-overlays#straight-arrows) trend line.
...
This example uses color-coded [`<s:callout>`](figure-overlays#callouts-with-leader-lines) components with curved leader lines to annotate an R histogram. Each callout uses an explicit `fill` to distinguish the annotations:
...
```svg overlay
<svg viewBox="0 0 600 400" xmlns:s="https://stencila.io/svg">
  <s:halo cx="390" cy="30" r="16"/>
  <s:callout x="440" y="30" label="Peak"/>
  <s:arrow x="170" y="240" to-x="238" to-y="165" stroke="green" stroke-width="3" label="Rising trend" label-position="below" fill="green"/>
</svg>
```

A Plotly chart with an overlay highlighting the peak with a [`<s:halo>`](figure-overlays#halo) and [`<s:callout>`](figure-overlays#callout), and an [`<s:arrow>`](figure-overlays#straight-arrows) indicating the rising trend.
...
Each subfigure can carry its own overlay. The overlay on each panel is scoped to that subfigure's content area. Here an [`<s:roi-rect>`](figure-overlays#roi-rectangle) highlights a rectangular region on the left panel and an [`<s:roi-ellipse>`](figure-overlays#highlighting-regions) highlights an elliptical region on the right:
...
In this example the parent overlay draws a connecting [`<s:arrow>`](figure-overlays#straight-arrows) between the two panels, while each subfigure uses its own overlay — a [`<s:halo>`](figure-overlays#halo) with [`<s:callout>`](figure-overlays#callout) on the left and an [`<s:roi-rect>`](figure-overlays#roi-rectangle) on the right. The parent overlay uses figure-level coordinates rather than either panel's local pixel coordinates, so its `viewBox` describes the overall two-panel layout rather than the dimensions of an individual image:
...
This example adds bottom padding so a [`<s:scale-bar>`](figure-overlays#scale-bar) can sit below the image instead of covering it:
...
Padding also works on parent figures with grid layouts. Here the parent figure uses bottom padding so [`<s:callout>`](figure-overlays#callouts-with-leader-lines) leader lines can point into each panel from a shared label below the grid:
...
<svg viewBox="0 0 824 306" xmlns:s="https://stencila.io/svg">
  ...
  <s:callout x="370" y="290" label="" to-x="610" to-y="90" dy="-25" />
</svg>
```
