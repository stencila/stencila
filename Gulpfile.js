    ...
    .pipe(through2.obj((file, enc, cb) => {
      ...
      function walk (node) {
        ...
        for (let [key, child] of Object.entries(node)) {
          if (key === '$ref' && typeof child === 'string') node[key] = child.replace('.yaml', '.json')
          walk(child)
        }
      }
      walk(schema)
    ...
    .pipe(through2.obj((file, enc, cb) => {
      (async function () {
        ...
        if (file.basename.includes('invalid')) {
          if (valid) {
            throw new Error(`? Woops. "${relativePath}" is supposed to be invalid, but it's not.`)
          } else {
            ...
            for (let error of errors) {
              if (!contents.includes(error.message)){
                throw new Error(`💣  Oh, oh, "${relativePath}" is expected to contain the comment "${error.message}".`)
              }
            }
          }
        } else {
...
exports.build = series(clean, jsonschema, parallel(check, test), parallel(jsonld, typescript))
exports.watch = () => watch(['schema/*', 'examples'], { ignoreInitial: false }, exports.build)
exports.clean = clean
