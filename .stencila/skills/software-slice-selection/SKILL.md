...
description: Mark the just-completed slice (if any), update the completed slices list, select the next unfinished slice from a delivery plan based on phase ordering and dependency constraints, and report whether more slices remain. Combines completion tracking with next-slice selection in a single step. Reads plans from .stencila/plans/, parses phase and slice structure, validates package references against the codebase, and reports the selected slice details or signals that all work is complete.
keywords:
  - slice selection
  - delivery plan
  - TDD slice
...
Mark the just-completed slice (if any), update the completed slices list, and select the next unfinished slice from a software delivery plan stored in `.stencila/plans/`. Given a just-finished slice name and a list of already-completed slices, append the finished slice, identify the next slice to work on based on phase ordering and dependency constraints, validate its package references against the codebase, and report the slice details along with the updated completed list. If all slices are complete, report that instead.
...
| Output                   | Description                                                                      |
|--------------------------|----------------------------------------------------------------------------------|
| Updated completed slices | The completed slices list with the just-finished slice appended (if provided)     |
...
1. Mark the just-completed slice (if provided):
   - If a just-completed slice name is provided and non-empty:
     ...
     - If the just-completed slice is not already in the list, append it
...
5. Select the next unfinished slice:
...
When all slices are complete, omit the Scope, Acceptance criteria, and Packages fields:
...
### Example 3: Mark completion crossing a phase boundary
...
### Example 4: Mark final completion — all slices done
