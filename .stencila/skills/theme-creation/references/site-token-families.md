# Site token family reference

Use this file for verified site-theme terminology, component families, application guidance, and the small set of exact names that are especially easy to misuse.
...
These exact names are useful reference anchors when responding without a live CLI query, but prefer the CLI for complete current inventories:
...
2. Use the CLI output to pick exact token names for the specific site surface being themed.
3. Use this reference for terminology, family boundaries, naming quirks, and application guidance that the raw token list does not explain.
4. If the needed family is not clear, describe the surface and intended effect instead of guessing an exact token name.
...
Localized from Stencila site theme token documentation when this skill was prepared.
