...
test.Datatable.name <- function(){
  checkEquals("t1",dt1$name())
}

test.Datatable.dimensions <- function(){
...
test.Datatable.colnames <- function(){
  checkEquals("a",dt1$colname(0))
  checkEquals(c("a","b","c"),dt1$colnames())
}

test.Datatable.types <- function(){
  checkEquals("Integer",dt1$type(0))
  checkEquals(c("Integer","Real","Text"),dt1$types())
}

test.Datatable.head <- function(){
  dt1$head()
  checkEquals(1.1,dt1$head(1)$value(0,1))
  checkEquals('a',dt1$head(1)$value(0,2))
}

test.Datatable.tail <- function(){
  dt1$tail()
  checkEquals(3.3,dt1$tail(1)$value(0,1))
}

test.Datatable.subscript.int.int <- function(){
...
test.Datatable.from_dataframe <- function(){
  df = data.frame(
    a = 1:5,
    b = c('a','b','c','d','e'),
  ...
  checkEquals(c('a','b','c'),dt$colnames())
