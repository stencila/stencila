...
Agents are specified in Markdown files with a YAML header (surrounded by three dashes i.e. `---`), the system prompt, a thematic break (another set of three dashes), and the user prompt. i.e.
...
Several header fields affect the which instructions an agent will execute on:

- `preference-rank`: The relative rank of the preference for the agent (0-256). Agents with a higher rank will be checked for an instruction match before those with a lower rank.
...
These parameters can be specified in the agent header and will be passed on to the LLM via it's API:
...
The user prompt in the agent Markdown file is a Jinja template so you can use [this syntax](https://docs.rs/minijinja/latest/minijinja/syntax/index.html) to alter the prompt based on the context of the instruction. e.g.
