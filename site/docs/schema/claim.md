...
Key properties include `claimType`, `label`, and `content`. The `claimType`
is intentionally optional: claims are often authored generically (e.g. a
plain `claim` block) and a more specific type such as `Statement` or
`Theorem` added only when known.
...
During property-based (a.k.a generative) testing, the properties of the `Claim` type are generated using the following strategies.
