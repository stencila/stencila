---
title: Graph
description: A directed graph relating Stencila nodes, used for provenance, reactivity, and similar relationship graphs.
---

This is a graph type used in Stencila for representing provenance, reactivity
and other graphs.

The `subject` identifies the real resource or scoped document node that the
graph was generated for. It should be an absolute URI with an optional fragment
for a scoped node, following the form `scheme://resource` or
`scheme://resource#fragment`, such as `file://report.md` or
`file://report.md#fig1`.

Graph nodes use durable graph-local `id` values and edges are directed
resource-flow relationships. Edge direction is always from upstream
dependency, source, or component to downstream dependant, result, or
containing node. Concrete operations that caused an edge may be attached to
the edge as `actions`; static analysis, inference, and direct observations are
recorded as edge evidence. The `edges` property is required for shape
stability, but may be an empty array for a single-node graph or a graph whose
relationships have not yet been collected.

The graph inherits creative-work metadata such as `dateCreated`,
`dateModified`, `version`, `authors`, `contributors`, and `references`.
Extending `CreativeWork` is intentional: provenance graphs may themselves be
authored, versioned, cited, funded, or published as reusable research objects,
even when many graphs only use a small subset of that metadata.


# Properties

The `Graph` type has these properties:

| Name             | Description                                                                                                             | Type                                                                              | Inherited from                       |
| ---------------- | ----------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- | ------------------------------------ |
| `subject`        | An absolute URI identifying the resource or scoped document node that the graph was generated for.                      | [`String`](./string.md)                                                           | -                                    |
| `nodes`          | The nodes in the graph.                                                                                                 | [`GraphNode`](./graph-node.md)*                                                   | -                                    |
| `edges`          | The directed edges in the graph.                                                                                        | [`GraphEdge`](./graph-edge.md)*                                                   | -                                    |
...
The `Graph` type is related to these types:
...
The `Graph` type is represented in:

- [JSON-LD](https://stencila.org/Graph.jsonld)
- [JSON Schema](https://stencila.org/Graph.schema.json)
- Python class [`Graph`](https://github.com/stencila/stencila/blob/main/python/stencila_types/src/stencila_types/types.py)
- Rust struct [`Graph`](https://github.com/stencila/stencila/blob/main/rust/schema/src/types/graph.rs)
- TypeScript class [`Graph`](https://github.com/stencila/stencila/blob/main/ts/src/types/Graph.ts)

***

This documentation was generated from [`Graph.yaml`](https://github.com/stencila/stencila/blob/main/schema/Graph.yaml) by [`docs_types.rs`](https://github.com/stencila/stencila/blob/main/rust/schema-gen/src/docs_types.rs).
