...
Maps local paths to remote service URLs. The key is the local path (file, directory, or pattern), and the value can be:
...
Example: ``{ url = "https://...", watch = "w123"  exec}

### Spread

Spread configuration for multi-variant pushes

Example: `", arguments = { region = ["north", "south"] } }`{ service = "gdoc", title = "Report {region exec}
...
If this remote is being watched for automatic synchronization, this field contains the watch ID. Watch configuration (direction, PR mode, debounce) is stored in Stencila Cloud and queried via the API.
...
Placeholders like ``{param exec} are replaced with arguments. Example: "Report - {region}"
...
Keys are parameter names, values are arrays of possible values. Example: ``{ region = ["north", "south"], species = ["A", "B"]  exec}
