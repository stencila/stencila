...
Stencila manifests also identify how the claim was generated in
`claim_generator_info`. This is related to trust, but it is not the same as
certificate trust:

- `Stencila CLI` means the claim was generated locally and signed locally.
- `Stencila Cloud Signing Service` means the claim was generated by a client
  and signed by Stencila Cloud.
- `Stencila Cloud` means the claim was generated and signed by Stencila Cloud.

The last case is stronger evidence about the provenance snapshot because the
Cloud service generated it before signing. A Cloud-signed local render still has
a Cloud signature, but the client supplied the provenance snapshot.
...
## Stencila Cloud Signers

For rendered outputs, Stencila can ask Stencila Cloud to sign the manifest:

```sh
stencila render article.smd article.pdf --credentials --credentials-signer cloud
```

Cloud signing uses the Stencila Cloud signing service at
`https://c2pa.stencila.cloud/v1/sign`. The route is a Stencila extension hosted
beside the C2PA Soft Binding Resolution API; it is not part of the official
C2PA API.

Use Cloud signing when the intended verifier should recognize an organizational
Stencila Cloud signing identity rather than a local self-signed identity. As
with any signer, recognition still depends on the verifier's trust policy.
