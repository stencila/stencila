...
import { BlockContext, LeafBlockParser, MarkdownConfig, LeafBlock, Element } from '@lezer/markdown'
...
const openingIfStatement = { name: "OpeningIfStatement", style: customTags.base }
...
const closingDelimiter = { name: "ClosingColonDelimiter", style: customTags.base }
...
const createDelimiter = 
  (cx: BlockContext, start: number, length: number): Element =>
    cx.elt(delimiterMark.name, start, start + length)

const createKeyWordEl = 
  (cx: BlockContext, start: number, length: number): Element => 
    cx.elt(keywordMark.name, start, start + length)
...
class OpeningIfParser implements LeafBlockParser {
  ...
  finish(cx: BlockContext, leaf: LeafBlock): boolean {
    ...
    cx.addLeafElement(
      leaf,
      cx.elt(openingIfStatement.name, leaf.start, getLeafEnd(leaf), elements)
    )
...
const StencilaColonSyntax: MarkdownConfig = {
  ...
  parseBlock: [
    ...
    },
    {
      ...
      endLeaf: (_, line) =>
        !delimiterElse.test(line.text)
    },
    {
      name: closingDelimiter.name,
      leaf: (_, leaf) => 
        closeDelimiter.test(leaf.content.trim()) ? new ClosingDelimiterParser() : null,
