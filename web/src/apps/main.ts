...
export class App extends LitElement {
  ...
  override render() {
    return html`
      ...
      <div class="h-screen flex flex-col">
        <main role="main" class="flex-grow px-4 py-8 w-full justify-center flex">
          <div class="bg-white border border-grays-mid container p-4 mx-0 h-full shadow-[0_0_8px_rgba(0,0,0,.035)]">
            here
            ${this.renderView()}
          </div>
        </main>
  ...
  private renderHeader() {
    return html`<header class="sticky w-full top-0 left-0 z-30 h-16 drop-shadow-[0_2px_0_#edf2f7] border-t-[3px] bg-white border-t-brand-blue p-4 font-[Lato]">
        
        <nav class="container mx-auto flex justify-items-center">
          <a href="/"><img src="${logo}" alt="Stencila logo" width="28" height="28" /></a>
        </nav>
      </header>`
  }

  private renderFooter() {
    return html`<footer class="bg-brand-blue px-4 py-6 text-white font-['Lato']">
      <div class="container mx-auto">
        <p class="text-sm my-0">
          &copy; 2023 Stencila Ltd.
        </p>
      </div>
    </footer>`
  }
